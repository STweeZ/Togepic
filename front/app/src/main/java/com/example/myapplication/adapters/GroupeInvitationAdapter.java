...
public class GroupeInvitationAdapter extends ArrayAdapter {

    private int id;

    public GroupeInvitationAdapter(Context context, List invitations, int id) {
        ...
        acceptInvitation.setOnClickListener(l -> {
            RestClient.makeRequest("POST",
                    getContext().getString(R.string.api_rest_groupe)
                            + id + getContext().getString(R.string.api_rest_user)
                            + invitation.getId(),
                    null,
                    getContext(),
